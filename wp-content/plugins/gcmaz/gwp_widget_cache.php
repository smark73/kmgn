...
Description: A plugin to cache WordPress Widgets using the Transients API, based on this tutorial http://generatewp.com/?p=10132
              Modified to be reversed from default (check widgets TO cache  (default = check widgets NOT TO cache))
Version: 1.1
Author: Ohad Raz
...
class GWP_Widget_cache{
     ...
     * @var int
     */
    public $cache_time = 60*60; // 1 hour
    ...
    function _cache_widget_output($instance, $widget, $args){
        ...
        if(empty($instance['wc_cache']) || $instance['wc_cache'] == null)
            return $instance;
    ...
    function in_widget_form($t,$return,$instance){
        ...
        <p>
            ...
            <label for="<?php echo $t->get_field_id('wc_cache'); ?>"><?php _e('Cache this widget?'); ?></label>
        </p>
        <?php
    }
